defmodule Airbrakex.Notifier do
  ...
  defp add_context(payload, nil) do
    payload |> Dict.put(:context, %{environment: Application.get_env(:airbrakex, :environment, :production)})
  end

  defp add_context(payload, context) do
    if !context[:environment] do
      context = context |> Dict.put(:environment, Application.get_env(:airbrakex, :environment, :production))
    end
